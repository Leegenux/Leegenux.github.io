...
With these 4 shortcuts, you are able to do some reverse engineering on easy crackmes like one that I'm gonna show you.
...
- Directly modification : That's directly making changes in the **Disassembler Window**.
- Create a new string somewhere else and make it work instead of the original one.

> To know more details about the GUI. Refer to [here](https://www.oreilly.com/library/view/practical-malware-analysis/9781593272906/ch10s02.html)
...
What need to be emphasized is that don't forget to save your changes to the binary.
...
Before debugging, you should first take a glance at what this file is about. It turns out that a password is required or you get nothing. Then load it in the debugger. Do step over until you see the prompt `What's your password?`. You are already in the main function. Trace back and at some point you see this: ![](https://ws3.sinaimg.cn/large/005BYqpgly1g1ilhjin0aj30iy0bvgls.jpg)

It jumps to the password section. Before it there are `MSVCR120.__winitenv` and `MSVCR120.exit`. So we can assure that this is the main function entry.
...
As you can see, there are no steps of clearing stack right behind the `CALL`. According to the description in the **X86 Calling Conventions** section. It uses `stdcall`. Registers play no part in passing parameters to callee.

### What API does the program calls during storing the decrypted data ?

Obviously according to the image: `KERNEL32.CreateFileA`, `KERNEL32.WriteFile` and `KERNEL32.CloseHandle`.
